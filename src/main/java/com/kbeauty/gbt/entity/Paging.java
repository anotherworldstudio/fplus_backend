...
public class Paging {
	...
	private Condition condition;       // 앞서 생성한 Criteria를 주입받는다.
	...
	private void calcData() {
		endPage = (int) (Math.ceil(condition.getPage() / (double) displayPageNum) * displayPageNum);
		
		startPage = (endPage - displayPageNum) + 1;
		
		int tempEndPage = (int) (Math.ceil(totalCount / (double) condition.getPerPageNum()));
		...
		next = endPage * condition.getPerPageNum() >= totalCount ? false : true;
	}
	...
	public String toString() {
		return "Paging [totalCount=" + totalCount + ", displayPageNum=" + displayPageNum + ", startPage=" + startPage
				+ ", endPage=" + endPage + ", prev=" + prev + ", next=" + next + ", condition=" + condition + ", page="
				+ page + "]";
	}
}
